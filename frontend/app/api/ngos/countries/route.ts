export async function GET() {
  try {
    console.log(`[DEBUG] NGO countries API called`);
    console.log(`[DEBUG] BACKEND_URL env var: ${process.env.BACKEND_URL}`);
    
    const backendUrl = new URL("/available-ngo-countries", process.env.BACKEND_URL);
    ...
    console.log(`[DEBUG] NGO countries data length:`, data?.data?.length || 0);
    ...
    console.error("[DEBUG] NGO countries API error:", error);
